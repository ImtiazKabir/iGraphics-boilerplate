...
The parameter `slices` is the number of slices to draw an ellipse or a circle. The more the `slices` the smoother the shape, but the slower the runtime! If you want to use the default behaviour of iGraphics then pass `-1` in place of `slices`. Actually any negative value would do.
